...
function render_apartment_ownwer_tenant_page() {
    ?>
                ...
                <div class="panel panel-white"><!--PANEL-WHITE-->
                    <div class="panel-body"><!--PANEL BODY-->
                            ...
                            <h2 class="nav-tab-wrapper">
                                <a href="?page=amgt-apartment-ownwer_tenant&amp;tab=ownerlist" class="nav-tab nav-tab-active">
                                <span class="dashicons dashicons-menu"></span>Owner/Tenants Essentials List</a>
                                
                                                                    <a href="?page=amgt-apartment-ownwer_tenant&amp;tab=add_owner_tenant" class="nav-tab ">
                                <span class="dashicons dashicons-plus-alt"></span> Add Owner/Tenants Essentials</a>
                                                            </h2>     

        <form method="post"> <table id="owner_list" class="display dataTable dtr-inline" cellspacing="0" width="100%" role="grid" aria-describedby="document_list_info" style="width: 100%;"><!---form_list TABLE--->
                         <thead>
...
?>   
        ...
        <td class="title sorting_1"><a href="?page=amgt-apartment-ownwer_tenant&tab=add_owner_tenant&action=edit&form_id=<?php echo $post_id; ?>"><?php the_title() ?></a></td>
    ...
    $first_five_words = implode(' ', array_slice($words, 0, 12));?>
        ...
        <td class="action">
           <a href="?page=amgt-apartment-ownwer_tenant&amp;tab=add_owner_tenant&amp;action=edit&amp;form_id=<?php echo $post_id; ?>" class="btn btn-info">Edit</a>
         
            <a href="?page=amgt-apartment-ownwer_tenant&amp;action=delete&amp;form_did=<?php echo $post_id; ?>" class="btn btn-danger" onclick="return confirm('Do you really want to delete this record?');">
            Delete</a>
...
?>
           <?php if($form_v!=""):?> <a target="_blank" href="<?php echo get_post_meta($post_id, 'repeater_file', true); ?>" class="btn btn-default"> <i class="fa fa-eye"></i> View Owner/Tenants Essentials</a><?php endif; ?>
            ...
            <?php if($form_li):?> <a target="_blank" href="<?php echo get_post_meta($post_id, 'repeater_url', true); ?>" class="btn btn-default"> <i class="fa fa-eye"></i> View Owner/Tenants Essentials URL</a><?php endif; ?>
    ...
    if ($post_id > 0 && get_post_status($post_id)) {
        ...
        //}
    ...
    else
            {
             ...
             $taxonomy_term_ids = wp_set_object_terms($post_id, 'Owner/Tenants Essentials', 'tower_taxonomies');
            ...
            if (is_wp_error($taxonomy_term_ids)) {
                ...
                // Handle successful post creation and taxonomy assignment
                       wp_redirect(admin_url('admin.php?page=amgt-apartment-ownwer_tenant&tab=ownerlist'));
                      }
    } 
}
...
// Check if the file is writable
 ...
 wp_delete_post($post_id, true); // Set to true to force delete permanently
    ...
    if (isset($_POST['selected_id']) && is_array($_POST['selected_id']) && !empty($_POST['selected_id'])) {
        ...
        wp_redirect(admin_url('admin.php?page=amgt-apartment-form&tab=formlist'));
        exit;
    } else {
...
?>
   ...
   <form name="add_owner_tenant" action="" method="post" class="form-horizontal" id="add_owner_tenant" enctype="multipart/form-data">
    ...
    <div id="repeater_fields">
       <?php if (!empty($post_title) ) : ?>
        ...
        <div class="repeater_field">
           ...
           <label>Owner/Tenants Essentials title <span class="require-field">*</span></label> <input type="text" name="repeater_title[]"  value="<?php echo esc_attr($post_title ?? ''); ?>">
            <label>Owner/Tenants Essentials Description</label>  <input type="textarea" name="repeater_content[]"  value="<?php echo esc_textarea($post_content ?? ''); ?>  ">
...
?>
        <p><a href="<?php echo $repeater_file; ?>" target="_blank" class="btn btn-default"><i class="fa fa-eye"></i>View Owner/Tenants Essentials</a></p>
...
<?php else : ?>
    <div class="repeater_field">
       <label>Owner/Tenants Essentials title <span class="require-field">*</span></label> <input type="text" name="repeater_title[]"  class="upload_file" value="<?php echo esc_attr($post_title ?? ''); ?>" required>
       <label>Owner/Tenants Essentials Description</label> <input type="text" name="repeater_content[]"  value="<?php echo esc_textarea($post_content ?? ''); ?>" class="upload_contet" >
...
<?php endif; ?>
    ...
    <div class="form-group">
        <div class="mb-3 row">  
            ...
            <div class="col-sm-12">              
                <p id="add_new_document_entry_frontend" class="btn btn-default" name="add_new_entry" onclick="addRepeaterField()">Add More Owner/Tenants Essentials</p>
            </div>
        </div>
    </div>
    ...
    <input type="hidden" name="_wp_http_referer" value="/wp-admin/admin.php?page=amgt-legal-documents&amp;tab=add_document">
    <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
        <input type="submit" value="Add Form" name="save_owner" class="form_btn btn btn-success">
    </div>
</form>
</div></div>
        </div>
