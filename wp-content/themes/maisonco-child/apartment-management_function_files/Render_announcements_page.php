...
function render_apartment_announcements_page() {
    ?>
                ...
                <div class="panel panel-white"><!--PANEL-WHITE-->
                    <div class="panel-body"><!--PANEL BODY-->
                            ...
                            <h2 class="nav-tab-wrapper">
                                <a href="?page=amgt-apartment-announcements&amp;tab=announcementslist" class="nav-tab nav-tab-active">
                                <span class="dashicons dashicons-menu"></span>Announcements List</a>
                                
                                                                    <a href="?page=amgt-apartment-announcements&amp;tab=add_announcements" class="nav-tab ">
                                <span class="dashicons dashicons-plus-alt"></span> Add Announcements</a>
                                                            </h2>     

        <form method="post"> <table id="announcements_list" class="display dataTable dtr-inline" cellspacing="0" width="100%" role="grid" aria-describedby="document_list_info" style="width: 100%;"><!---form_list TABLE--->
                         <thead>
...
?>   
        ...
        <td class="title sorting_1"><a href="?page=amgt-apartment-announcements&tab=add_announcements&action=edit&form_id=<?php echo $post_id; ?>"><?php the_title() ?></a></td>
    ...
    $first_five_words = implode(' ', array_slice($words, 0, 12));?>
        ...
        <td class="action">
           <a href="?page=amgt-apartment-announcements&amp;tab=add_announcements&amp;action=edit&amp;form_id=<?php echo $post_id; ?>" class="btn btn-info">Edit</a>
         
            <a href="?page=amgt-apartment-announcements&amp;action=delete&amp;form_did=<?php echo $post_id; ?>" class="btn btn-danger" onclick="return confirm('Do you really want to delete this record?');">
            Delete</a>
    ...
    if ($post_id > 0 && get_post_status($post_id)) {
        ...
        //}
    ...
    else
            {
             ...
             $taxonomy_term_ids = wp_set_object_terms($post_id, 'Announcements', 'tower_taxonomies');
            ...
            if (is_wp_error($taxonomy_term_ids)) {
                ...
                // Handle successful post creation and taxonomy assignment
                       wp_redirect(admin_url('admin.php?page=amgt-apartment-announcements&tab=announcementslist'));
                      }
    } 
}
...
?>
   ...
   <form name="add_announcements" action="" method="post" class="form-horizontal" id="add_announcements" enctype="multipart/form-data">
    ...
    <div id="repeater_fields">
       <?php if (!empty($post_title) ) : ?>
        ...
        <div class="repeater_field">
           ...
           <label>Announcements title <span class="require-field">*</span></label> <input type="text" name="repeater_title[]"  value="<?php echo esc_attr($post_title ?? ''); ?>">
            <label>Announcements Description</label>  <input type="textarea" name="repeater_content[]"  value="<?php echo esc_textarea($post_content ?? ''); ?>  ">
...
?>
        <p><a href="<?php echo $repeater_file; ?>" target="_blank" class="btn btn-default"><i class="fa fa-eye"></i>View Announcements</a></p>
...
<?php else : ?>
    <div class="repeater_field">
       <label>Announcements title <span class="require-field">*</span></label> <input type="text" name="repeater_title[]"  class="upload_file" value="<?php echo esc_attr($post_title ?? ''); ?>" required>
       <label>Announcements Description</label> <input type="text" name="repeater_content[]"  value="<?php echo esc_textarea($post_content ?? ''); ?>" class="upload_contet" >
...
<?php endif; ?>
    ...
    <div class="form-group">
        <div class="mb-3 row">  
            ...
            <div class="col-sm-12">              
                <p id="add_new_document_entry_frontend" class="btn btn-default" name="add_new_entry" onclick="addRepeaterField()">Add More Announcements</p>
            </div>
        </div>
    </div>
    ...
    <input type="hidden" name="_wp_http_referer" value="/wp-admin/admin.php?page=amgt-announcements&amp;tab=add_announcements">
    <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
        <input type="submit" value="Add Announcements" name="save_owner" class="form_btn btn btn-success">
    </div>
</form>
</div></div>
        </div>
