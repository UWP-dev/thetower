...
function osf_get_google_fonts() {
    $content = file_get_contents(MAISONCO_CORE_PLUGIN_DIR . 'webfonts.json');
...
if (!function_exists('osf_is_elementor_pro_activated')) {
    function osf_is_elementor_pro_activated() {
    ...
    function osf_scrape_instagram($username, $slice = 9) {
        ...
        if (false === ($instagram = get_transient('otf-instagram-media-new-' . sanitize_title_with_dashes($username)))) {
            ...
            if (is_wp_error($remote))
                return new WP_Error('site_down', esc_html__('Unable to communicate with Instagram.', 'maisonco-core'));

            if (200 != wp_remote_retrieve_response_code($remote))
                return new WP_Error('invalid_response', esc_html__('Instagram did not return a 200.', 'maisonco-core'));
            ...
            if (!$insta_array)
                return new WP_Error('bad_json', esc_html__('Instagram has returned invalid data.', 'maisonco-core'));
            ...
            if (isset($insta_array['entry_data']['UserProfile'][0]['userMedia'])) {
                ...
                return new WP_Error('bad_json_2', esc_html__('Instagram has returned invalid data.', 'maisonco-core'));
            }


            if (!is_array($images))
                return new WP_Error('bad_array', esc_html__('Instagram has returned invalid data.', 'maisonco-core'));
            ...
            switch ($type) {
                ...
                case 'old_2':
                    foreach ($images as $image) {
                        ...
                        $caption = esc_html__('Instagram Image', 'maisonco-core');
        ...
        if (!empty($instagram)) {
            ...
            return new WP_Error('no_images', esc_html__('Instagram did not return any images.', 'maisonco-core'));
        }
    }
}
...
function time_elapsed_string($datetime, $full = false) {
    ...
    $string = array(
        'y' => __('year', 'maisonco-core'),
        'm' => __('month', 'maisonco-core'),
        'w' => __('week', 'maisonco-core'),
        'd' => __('day', 'maisonco-core'),
    );
    ...
    return $string ? implode(', ', $string) . __(' left', 'maisonco-core') : __('just now', 'maisonco-core');
}

if (!function_exists('osf_get_page_by_title')) {
    function osf_get_page_by_title($title,$output = OBJECT, $post_type = 'page') {
        $query = new WP_Query(
            [
                'post_type' => $post_type,
                'title'     => $title,
