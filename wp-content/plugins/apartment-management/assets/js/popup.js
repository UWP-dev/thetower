jQuery(document).ready(function($) {
    // Keyup event handler for the search filter
    ...
    $("body").off("click", "#addremove").on("click", "#addremove", function(event) {
        jQuery("#myModal_add_building").css({ "display": "none" });
        ...
        if (model == "designation_cat") {
            jQuery("#myModal_add_member").css({ "display": "none" });
        }
        var curr_data = {
            action: 'MJ_amgt_add_or_remove_category',
            model: model,
            dataType: 'json'
        };
    ...
    $("body").on("click", ".close-btn", function() {
        ...
        var model = $(this).attr('model');
        if (model == "building_unit_category") {
            $("#myModal_add_building").css({ "display": "block" });
            $(".category_list").empty();
            $('.popup-bg').hide(); // hide the overlay			
        } else if (model == "designation_cat") {
            $("#myModal_add_member").css({ "display": "block" });
            $(".category_list").empty();
            $('.popup-bg').hide(); // hide the overlay	
        } else {
            $(".category_list").empty();
    ...
    //Category Add and Remove  NEW
    $("body").off("click", "#addremove_new").on("click", "#addremove_new", function(event) {
        jQuery("#myModal_add_building").css({ "display": "none" });
        event.preventDefault(); // disable normal link function so that it doesn't refresh the page
        var docHeight = $(document).height(); //grab the height of the page
        var scrollTop = $(window).scrollTop();
        ...
        var curr_data = {
            action: 'MJ_amgt_add_or_remove_category_new',
            model: model,
            dataType: 'json'
        };
    ...
    $("body").on("click", ".close-btn_new", function() {
    ...
    $("body").on("click", ".btn-delete-cat", function() {
        ...
        if (confirm(language_translate.add_remove)) {
            var curr_data = {
                action: 'MJ_amgt_remove_category',
                model: model,
                cat_id: cat_id,
                dataType: 'json'
            };
    ...
    $("body").on("click", "#btn-add-cat", function() {
        ...
        var model = $(this).attr('model');
        if (category_name == 0) {
            alert(language_translate.category_alert);
            return false;
        }
        if (category_name != "") {
            var curr_data = {
                action: 'MJ_amgt_add_category',
                model: model,
    ...
    $("body").on("change", "#badge_id", function() {
        $('#staff-data').html('');
        var selection = $(this).val();

        var curr_data = {
            action: 'MJ_amgt_load_staff_checkin_data',
            badge_id: selection,
            dataType: 'json'
        };
        $.post(amgt.ajax, curr_data, function(response) {
    ...
    jQuery("body").on("change", ".unit_categorys", function() {
        ...
        var select_message1 = "<option value>Select Member</option>";
    ...
    $("body").on("change", "#facility_id", function() {
        $('#select_facility_block').html('');
        var selection = $(this).val();

        var curr_data = {
            action: 'MJ_amgt_facility_booking_period',
    ...
    $("body").on("click", ".view-service", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_service_complaint',
    ...
    $("body").on("click", ".view-unit-document", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_unit_document_view',
            unit_name: unit_name,
            building_id: building_id,
            dataType: 'json'
        };
    ...
    $("body").on('change', '#invoice', function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_load_invoice_amount',
            invoice_id: $(this).val(),
            dataType: 'json'
        };
    ...
    $("body").on('focus', '.facility_charge', function(event) {
        ...
        if (period_type == 'hour_type') {
            ...
            if (end_minutes < 10) eMinutes = "0" + eMinutes;
                ...
                var curr_data = {
                    action: 'MJ_amgt_count_facility_charge',
                    start: $('.start').val(),
                    end: $('.end').val(),
    ...
    $("body").on("click", "#generate_invoice", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_generate_invoice_form',
            dataType: 'json'
        };
    ...
    $("body").on("click", ".view-member", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_unit_member_view',
            unit_name: unit_name,
            building_id: building_id,
            dataType: 'json'
        };
    ...
    $("body").on("click", ".view-member-history", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_unit_member_history_view',
            unit_name: unit_name,
            building_id: building_id,
            dataType: 'json'
        };
    ...
    $("body").on("click", "#delete_unitmember", function() {
        ...
        if (confirm(language_translate.add_remove)) {
            var curr_data = {
                action: 'MJ_amgt_remove_unit_member',
                member_id: member_id,
    ...
    $("body").on("change", "#committee_member", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_load_member_designation',
    ...
    $("body").on("click", ".show-payment-popup", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_member_add_payment',
    ...
    jQuery("body").on("change", "#unit_name", function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_load_unit_measurements',
    ...
    jQuery("body").on("change", ".tax_selection", function() {
        ...
        var id = $(this).attr('id');
        var curr_data = {
            action: 'MJ_amgt_load_tax_amount',
            tax_id: selection,
            dataType: 'json'
        };
        $.post(amgt.ajax, curr_data, function(response) {
    ...
    $("body").on("change", ".unit_name", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_unit_wise_view_member',
            unit_name: unit_name,
    ...
    $("body").on("change", ".account_unit_name", function(event) {
        ...
        var curr_data = {
            action: 'MJ_amgt_account_unit_wise_view_member',
            unit_name: unit_name,
            building_category: building_category,
            unit_category: unit_category,
            dataType: 'json'
    ...
    $("body").on("click", '#add_new_document_entry', function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_load_document_html',
            dataType: 'json'
        };
    ...
    $("body").on("click", '#add_new_document_entry_frontend', function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_load_document_html_frontend',
            dataType: 'json'
        };
    ...
    jQuery("body").on("change", ".member_list", function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_member_wise_view_invoice',
            member_id: member_id,
            dataType: 'json'
        };
        $.post(amgt.ajax, curr_data, function(response) {
    ...
    $("body").on("change", 'input:radio[name="select_serveice"]', function() {
        var invoice_option = $(this).val();
        var curr_data = {
            action: 'MJ_amgt_invoice_option_html',
    ...
    $('.charges_category').on('change', function() {
        ...
        var curr_data = {
            ...
            action: 'MJ_amgt_generate_invoice_form_allmember',
            val: val,
            dataType: 'json'
        };
    ...
    jQuery("body").on("change", ".allready_occupied", function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_unit_allready_occupied',
            building_category: building_category,
            unit_category: unit_category,
            unit_name: unit_name,
            dataType: 'json'
    ...
    $("body").on("click", "#delete_occupied_unitmember", function() {
        ...
        if (confirm(language_translate.add_remove)) {
            var curr_data = {
                action: 'MJ_amgt_remove_allready_occupied_member',
                member_id: member_id,
    ...
    $("body").on("change", 'input:radio[name="charge_cal"]', function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_charge_cal_option_html',
    ...
    $("body").on("change", '.tax_div_clear', function() {
        var charge_cal_option = $(this).val();

        $('#charges_entry1').empty();

        var curr_data = {
            action: 'MJ_amgt_tax_div_html',
            charge_cal_option: charge_cal_option,
            dataType: 'json'
        };
    ...
    $("body").on("click", "#profile_change", function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_change_profile_photo',
            dataType: 'json'
        };
    ...
    $("body").on("click", ".visitor_details_search", function() {
        ...
        var curr_data = {
            action: 'MJ_amgt_load_visitor_data_by_id',
        ...
        $.post(amgt.ajax, curr_data, function(response) {
            var json_obj = $.parseJSON(response);
            if (json_obj[0] == '0') {
                alert(language_translate.no_record_found);
            } else {
    ...
    $("body").on("click", '#add_new_document_entry_member', function() {
        ...
        var curr_data = {
            ...
            action: 'MJ_amgt_load_document_html_member',
    ...
    // staff and member chage when select sloat // 

    $("body").on("change", '.form_sloat_select', function() {

        var sloat_id = $(this).val();
        var curr_data = {

            action: 'MJ_amgt_load_staff_select_option',
            sloat_id: sloat_id,
            dataType: 'json'
        ...
        $.post(amgt.ajax, curr_data, function(response) {
            ...
            var slot_type = $.parseJSON(response);
            if (slot_type == 'member') {
                $('.member-list').css('display', 'block');
                $('.staff-member-list').css('display', 'none');
            }
            if (slot_type == 'Staff') {
                $('.member-list').css('display', 'none');
                $('.staff-member-list').css('display', 'block');
            } else {
                $('.member-list').css('display', 'block');
                $('.staff-member-list').css('display', 'none');
            }
    ...
    $("body").on("change", '.building_information1', function() {

        var usert_type = $('.import_csv').val();
        // alert(usert_type);
        if (usert_type == 'member') {
            $('.building_information111').css('display', 'block');
        } else if (usert_type != 'member') {
            $('.building_information111').css('display', 'none');
        }
        return false;
    ...
    $("#delete_selected").on('click', function() {
        if ($('.sub_chk:checked').length == 0) {
            // alert('heloo');
            alert(language_translate.one_record_select_alert);
            return false;
        } else {
