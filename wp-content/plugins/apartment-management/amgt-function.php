...
function MJ_amgt_get_all_user_in_message()
{
	...
	$arg_gatekeeper = array(
		'role'=>'gatekeeper',
		'orderby' => 'meta_value',
		'meta_key'=> 'last_name',
		'order' => 'ASC'
	);
	$arg_administrator = array(
		'role'=>'administrator',
		'orderby' => 'meta_value',
		'meta_key'=> 'last_name',
		'order' => 'ASC'
	);
	$member=get_users($arg_member);
	$accountant = get_users($arg_account);
	$staff_member = get_users($arg_staff);
	$committee_member = get_users($arg_commitee);
	$gatekeeper = get_users($arg_gatekeeper);
	$admin = get_users($arg_administrator);
	...
	{ 
		if(!empty($value))
		{
			...
			foreach($value as $user)
			{
				...
				echo  '<option value="'.$user->ID.'">'.$last_name.' '.$first_name.'</option>';
					  ...
					  <div class="field_left">
						<div class="form_field">
						  ...
						  <input type="text"
								 ...
								 value="<?php esc_url($img);//esc_html_e( $gallery_data['image_url'][$i] ); ?>"/>
						</div>
					  </div>
			 
					  <div class="field_right image_wrap">
						<img src="<?php esc_url($img);//esc_html_e( $gallery_data['image_url'][$i] ); ?>" height="65" width="65" />
					  </div>
...
<?php 
				MJ_amgt_invoice_print($_REQUEST['invoice_id'],$_REQUEST['invoice_type']);
				exit;
	}
	...
	{ ?>
			<script>window.onload = function(){ window.print(); };</script>
			<?php 
			MJ_amgt_visiter_checkin_details_print($_REQUEST['visitor_checkin_id']);
			exit;
	}
	...
	<script>window.onload = function(){ window.print(); };</script>
	<?php 
		MJ_amgt_invoice_payment_receipt_print($_REQUEST['invoice_id'],$_REQUEST['member_id']);
		exit;
	}	
}
...
function MJ_amgt_pdf_init()
{
	if (is_user_logged_in ()) 
	{
		...
		{			
			MJ_amgt_invoice_pdf($_REQUEST['invoice_id'],$_REQUEST['invoice_type']);
			exit;
		}	
	}
}
...
function MJ_amgt_count_reply_item($id)
{
	...
	$inbox_sent_box =$wpdb->get_results("SELECT *  FROM $tbl_amgt_message where ((receiver = $user_id) AND (sender != $user_id)) AND (post_id = $id) AND (msg_status=0)");
	
	$reply_msg =$wpdb->get_results("SELECT *  FROM $amgt_message_replies where (receiver_id = $user_id) AND (message_id = $id) AND ((msg_status=0) OR (msg_status IS NULL))");
...
function MJ_amgt_count_unread_message($user_id)
{
	...
	$reply_msg =$wpdb->get_results("SELECT *  FROM $smgt_message_replies where (receiver_id = $user_id) AND ((msg_status=0) OR (msg_status IS NULL))");
...
function MJ_amgt_get_receiver_name_array($message_id,$sender_id,$created_date,$message_comment)
{
	...
	if (!empty($reply_msg)) {
		foreach ($reply_msg as $receiver_id) {
			$receiver_name[]=MJ_amgt_get_display_name($receiver_id->receiver_id);
		}
	}
...
function MJ_amgt_count_unread_message_admin($user_id)
{
	...
	$reply_msg =$wpdb->get_results("SELECT *  FROM $amgt_message_replies where (receiver_id = $user_id) AND ((msg_status=0) OR (msg_status IS NULL))");
...
function MJ_amgt_facility_booking_add_date_type_data($start_date,$end_date,$facility_id)
{
	...
	$facility_booking_data = $wpdb->get_results("SELECT * FROM $table_amgt_facility WHERE `start_date` <= '$start_date' AND `end_date` >= '$end_date' AND `facility_id`=$facility_id AND `period_type`='date_type'");
...
function MJ_amgt_facility_booking_edit_data_id($start_date,$end_date,$facility_id)
{
	global $wpdb;
	$table_amgt_facility = $wpdb->prefix. 'amgt_facility_booking';
	$facility_booking_data = $wpdb->get_results("SELECT id FROM $table_amgt_facility WHERE `start_date` <= '$start_date' AND `end_date` >= '$end_date' AND `facility_id`=$facility_id AND `period_type`='date_type'"); 
	//$id = array_column($facility_booking_data, 'id');
...
function MJ_amgt_facility_booking_edit_time_id($start_date,$start_time,$end_time,$facility_id)
{
	global $wpdb;
	$table_amgt_facility = $wpdb->prefix. 'amgt_facility_booking';
	$facility_booking_data = $wpdb->get_results("SELECT id FROM $table_amgt_facility WHERE `start_date`= '$start_date' AND `start_time` <='$start_time' AND `end_time` >= '$end_time' AND facility_id=$facility_id AND period_type='hour_type'"); 
	//$id = array_column($facility_booking_data, 'id');
	return $facility_booking_data['0']->id;
}
