...
<div class="panel-body panel-white"><!-- PANEL WHITE DIV -->
    <ul class="nav nav-tabs panel_tabs" role="tablist"><!-- PANEL_TABS -->
	  	<li class="<?php if($active_tab=='sloat-list'){?>active<?php }?>">
			<a href="?apartment-dashboard=user&page=parking-manager&tab=sloat-list" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'sloat-list' ? 'active' : ''; ?>">
	 ...
	 <?php if(($obj_apartment->role=='staff_member' || $obj_apartment->role=='gatekeeper') && ($user_access['add']=='1')){?>
        <li class="<?php if($active_tab=='add_sloat'){?>active<?php }?>">
		  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['sloat_id']))
			{ ?>
			<a href="?apartment-dashboard=user&page=parking-manager&tab=add_sloat&action=edit&sloat_id=<?php echo $_REQUEST['sloat_id'];?>" class="nav-link px-3 nav-tab <?php echo esc_html($active_tab) == 'visitor-checkin' ? 'nav-tab-active' : ''; ?>">
             <i class="fa fa"></i> <?php esc_html_e('Edit Slot', 'apartment_mgt'); ?></a>
			 <?php }
			else
			{ ?>
				<a href="?apartment-dashboard=user&page=parking-manager&tab=add_sloat" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'add_sloat' ? 'active' : ''; ?>">
		...
		<li class="<?php if($active_tab=='assigned-sloat-list'){?>active<?php }?>">
				<a href="?apartment-dashboard=user&page=parking-manager&tab=assigned-sloat-list" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'assigned-sloat-list' ? 'active' : ''; ?>">
		...
		<?php  if(($obj_apartment->role=='staff_member' || $obj_apartment->role=='gatekeeper') && ($user_access['add']=='1')){?> 
		   <li class="<?php if($active_tab=='assign_sloat'){?>active<?php }?>">
			  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && $_REQUEST['tab']=='assign_sloat')
				{ ?>
				<a href="?apartment-dashboard=user&page=parking-manager&tab=assign_sloat&action=edit&sloat_assign_id=<?php echo $_REQUEST['sloat_assign_id'];?>" class="nav-link px-3 nav-tab margin_top_10_res <?php echo esc_html($active_tab) == 'assign_sloat' ? 'nav-tab-active' : ''; ?>">
				 <i class="fa fa"></i> <?php esc_html_e('Edit Slot Assigned', 'apartment_mgt'); ?></a>
				 <?php }
				else
				{ ?>
					<a href="?apartment-dashboard=user&page=parking-manager&tab=assign_sloat" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'assign_sloat' ? 'active' : ''; ?>">
				...
				<table id="sloat_list" class="display" cellspacing="0" width="100%"><!---SLOT LIST TABLE--->
					...
					<tbody>
						...
						elseif($obj_apartment->role=='staff_member')
						{
							...
							if($own_data == '1')
							{  
								$sloatdata=$obj_parking->MJ_amgt_get_own_sloats($user_id);
							}
							else
							{
								$sloatdata=$obj_parking->MJ_amgt_get_all_sloats();
						...
						if(!empty($sloatdata))
						{
							...
							{?>
								<tr>
									...
									<td class="sloattype"><?php if($retrieved_data->sloat_type=='Staff') echo esc_html__('Staff','apartment_mgt'); else echo esc_html__('Member','apartment_mgt');?></td>
...
<div class="modal" id="myModal_add_member" tabindex="-1" aria-labelledby="myModal_add_building" role="dialog" aria-modal="true">
				...
				<form name="member_form" action="<?php echo admin_url('admin-ajax.php'); ?>"  method="post" class="form-horizontal" id="member_form">
					...
					<div class="form-group">
						<div class="mb-3 row">	
							...
							<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
								<input id="last_name" class="form-control validate[required,custom[onlyLetter_specialcharacter]] text-input" maxlength="50" type="text"  value="<?php if($edit){ echo esc_attr($result->last_name);}elseif(isset($_POST['last_name'])) echo $_POST['last_name'];?>" name="last_name">
							</div>
						</div>
					</div>
					...
					<div class="form-group">
							<div class="mb-3 row">	
								...
								<div class="col-sm-1">
									<div class="col-sm-1">
									<input id="committee_member" class="form-check-input text-input" type="checkbox" <?php if($edit==1 && $result->committee_member=='yes'){ echo "checked";}?> name="committee_member" 
									value="yes"></div>	
								</div>	
					...
					<!-- <div class="form-group">
						<div class="mb-3 row">	
							...
							<div class="col-sm-3">
								<input class="form-control validate[required,custom[onlyLetter_specialcharacter]]" maxlength="50" type="text"  name="state_name" 
					...
					<div class="form-group">
						<div class="mb-3 row">	
							...
							<div class="col-sm-3 margin_bottom_10_res">
								<input class="form-control validate[required,custom[onlyLetter_specialcharacter]]" maxlength="50" type="text"  name="country_name" 
...
<div class="modal" id="myModal_add_building" tabindex="-1" aria-labelledby="myModal_add_building" role="dialog" aria-modal="true">
    <div class="modal-dialog modal-lg"><!--MODAL-DIALOG---->
        <div class="modal-content"><!--MODAL CONTENT---->
			<div class="modal-header"><!--MODAL-HEADER---->
							...
							<div id="unit_name_entry">
									<div class="form-group">
										<div class="mb-3 row">	
											...
											<label class="col-sm-3 control-label form-label" for="unit_entry"><?php esc_html_e('Unit Size','apartment_mgt');?>(<?php if($unit_measerment_type =='square_meter'){
