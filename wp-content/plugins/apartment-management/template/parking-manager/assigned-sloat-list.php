				...
				<table id="assigned_sloat_list" class="display" cellspacing="0" width="100%"><!--ASSIGNED-SLOT-LIST TABLE--->
					...
					<tbody>
						...
						if($obj_apartment->role=='member')
						{
							...
							if($own_data == '1')
							{
								$assignedsloatdata=$obj_parking->MJ_amgt_get_own_assigned_sloats($user_id);
							}
							else
							{
						...
						if(!empty($assignedsloatdata))
						{
						   ...
						   ?>
							<tr>
								...
								<td class="sloattype"><?php if($sloat->sloat_type=='Staff') echo esc_html__('Staff','apartment_mgt'); else echo esc_html__('Member','apartment_mgt');?></td>
								<td class="building"><?php if(!empty($retrieved_data->building_id)){echo esc_html(get_the_title($retrieved_data->building_id));}else{echo "-";}?></td>
								<td class="member">
								<?php
