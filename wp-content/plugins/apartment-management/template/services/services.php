...
<div class="panel-body panel-white"><!-- PANEL BODY DIV -->
    <ul class="nav nav-tabs panel_tabs" role="tablist"><!--TABLIST-->
		<li class="<?php if($active_tab=='service-list'){?>active<?php }?>">
			<a href="?apartment-dashboard=user&page=services&tab=service-list" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'service-list' ? 'active' : ''; ?>">
	    ...
	    <li class="<?php if($active_tab=='add_service'){?>active<?php }?>">
		  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['service_id']))
			{ ?>
				<a href="?apartment-dashboard=user&page=services&tab=add_service&action=edit&service_id=<?php echo $_REQUEST['service_id'];?>" class="nav-link px-3 nav-tab <?php echo esc_html($active_tab) == 'add_service' ? 'nav-tab-active' : ''; ?>">
			...
			{ 
				...
				{ ?>
					<a href="?apartment-dashboard=user&page=services&tab=add_service" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'add_service' ? 'active' : ''; ?>">
        	...
        	<div class="table-responsive"><!--TABLE RESPONSIVE--> 
				<table id="service_list" class="display" cellspacing="0" width="100%"><!--SERVICE_LIST TABLE-->
					...
					if($obj_apartment->role=='member')
					{
						...
						if($own_data == '1')
						{
							$service_data= $obj_service->MJ_amgt_get_own_service($user_id);
						}
						else
						{
					...
					if(!empty($service_data))
					{
						...
						{  ?>
							<td class="action">
								...
								{  ?>
									<a href="?apartment-dashboard=user&page=services&tab=add_service&action=edit&service_id=<?php echo esc_attr($retrieved_data->service_id)?>" class="btn btn-info"> <?php esc_html_e('Edit', 'apartment_mgt' ) ;?></a>
									<?php
								}
								if($user_access['delete']=='1')
								{
								?>
									<a href="?apartment-dashboard=user&page=services&tab=service-list&action=delete&service_id=<?php echo esc_attr($retrieved_data->service_id);?>" class="btn btn-danger" onclick="return confirm('<?php esc_html_e('Do you really want to delete this record?','apartment_mgt');?>');">
