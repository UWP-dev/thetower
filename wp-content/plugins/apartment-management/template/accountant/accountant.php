<?php 
 $active_tab = sanitize_text_field(isset($_GET['tab'])?$_GET['tab']:'accountantlist'); //ACCOUNTANT LIST
...
{	
 ...
 //SAVE ACOOUNTANT DATA //
	...
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
	{
		...
		$result=$obj_member->MJ_amgt_delete_usedata($_REQUEST['accountant_id']);
		if($result)
		{
...
<div class="panel-body panel-white"><!--PANEL BODY DIV--> 
	<ul class="nav nav-tabs panel_tabs" role="tablist"><!--TABLIST--> 
			<li class="<?php if($active_tab=='accountantlist'){?>active<?php }?>">
				<a href="?apartment-dashboard=user&page=accountant&tab=accountantlist" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'accountantlist' ? 'active' : ''; ?>">
			...
			<li class="<?php if($active_tab=='add_accountant'){?>active<?php }?>">
				  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit')
					{ ?>
					<a href="?apartment-dashboard=user&page=accountant&tab=add_accountant&action=edit&accountant_id=<?php echo $_REQUEST['accountant_id'];?>" class="nav-link px-3 nav-tab margin_top_10_res <?php echo esc_html($active_tab) == 'add_accountant' ? 'nav-tab-active' : ''; ?>">
					...
					else
					{
						...
						{ ?>
							<a href="?apartment-dashboard=user&page=accountant&tab=add_accountant" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'add_accountant' ? 'active' : ''; ?>">
