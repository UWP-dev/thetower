...
$active_tab = sanitize_text_field(isset($_GET['tab'])?$_GET['tab']:'invoice-list');
...
if(isset($_POST['add_own_payment']))
{
	...
	{
		$result=$obj_account->MJ_amgt_add_own_payment($_POST);			
		if($result)
		{ 
...
if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'ipn')
{		
	...
	$obj_account->MJ_amgt_save_invoice_payment($paymentdata);	
}
if(isset($_POST['add_expense']))		
{		
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='edit')//EDIT ACCOUNTS
	{
		$result=$obj_account->MJ_amgt_add_expense($_POST);
		if($result)
		{
...
{	
	...
	{
		$result=$obj_account->MJ_amgt_save_charges_payment($_POST);
		if($result)
		{
...
{	
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='edit')
	{
		$result=$obj_account->MJ_amgt_save_charges_payment_all_member($_POST);
...
if(isset($_POST['add_charges_all_member_with_create_invoice']))	//ADD_CHARGES_ALL_MEMBER_WITH_CREATE_INVOICE	
{
	...
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='edit')
	{
		...
		if(empty($result_invoice_data))
		{
			...
			{
				$entry_value=$obj_account->MJ_amgt_get_entry_records($_POST);
			}
			elseif($_POST['charge_cal']=='measurement_charge')
			{
				$entry_value=$obj_account->MJ_amgt_get_entry_records_by_measurement($_POST);
			}
			...
			if(!empty($member_data))
			{
				...
				{							
					...
					if($_POST['amgt_charge_period']=='0')
					{
						...
						if($_POST['one_time_recurring_option'] == 1)
						{
							$start_date=$_POST['start_date_one_charge'];
							$end_date_add=$_POST['end_date_one_charge'];
						...
						{
							$invoice_data['start_date']=$_POST['start_date_one_charge'];
							$invoice_data['end_date']=$_POST['end_date_one_charge'];
						}
					}
					elseif($_POST['amgt_charge_period']=='1')
					{
			...
			{						
				...
				if(!empty($_POST['start_date_one_charge']))
				{
					$invoice_generate_date['invoice_start_date']=$_POST['start_date_one_charge'];
				}
				else
				{
...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
{
	...
	{
		$result=$obj_account->MJ_amgt_delete_invoice($_REQUEST['invoice_id']);
		if($result)
		{
	...
	{
		$result=$obj_account->MJ_amgt_delete_expense($_REQUEST['expense_id']);
		if($result)
		{
	...
	{
		$result=$obj_account->MJ_amgt_delete_payment($_REQUEST['payment_id']);
		if($result)
		{
	...
	{
		$result=$obj_account->MJ_amgt_delete_paid_charges($_REQUEST['pay_charges_id']);
		if($result)
		{
	...
	if($message == 1)
		...
		{?>
			<div id="message_template" class="alert_msg alert alert-success alert-dismissible" role="alert">
				...
				?>
				<button type="button" class="close btn-close float-end p-3 padding_07_rem" data-bs-dismiss="alert" aria-label="Close"></button>
			</div>
		<?php				
		}
	if($message == 5)
		...
		{?>
			<div id="message_template" class="alert_msg alert alert-success alert-dismissible" role="alert">
				...
				?>
				<button type="button" class="close btn-close float-end p-3" data-bs-dismiss="alert" aria-label="Close">X</button>
			</div>
		<?php				
		}	
}	
...
{ 
	...
	wp_redirect ( home_url().'?apartment-dashboard=user&page=accounts&message=4');

}	
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='cancel')
{
	wp_redirect ( home_url().'?apartment-dashboard=user&page=accounts&message=7');
}?>
<!-- POP UP CODE -->
<div class="popup-bg">
...
<div class="panel-body panel-white"><!-- PANEL WHITE-->
	 <ul class="nav nav-tabs panel_tabs" role="tablist">	
		 <li class="<?php if($active_tab=='invoice-list'){?>active<?php }?>">
				<a href="?apartment-dashboard=user&page=accounts&tab=invoice-list" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'invoice-list' ? 'active' : ''; ?>">
		...
		?>	
			<li class="<?php if($active_tab=='charges-list'){?>active<?php }?>">
				<a href="?apartment-dashboard=user&page=accounts&tab=charges-list" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'charges-list' ? 'active' : ''; ?>">
			...
			<li class="<?php if($active_tab=='add-charges'){?>active<?php }?>">
			  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['pay_charges_id']))
				{ ?>
				<a href="?apartment-dashboard=user&page=accounts&tab=add-charges&action=edit&pay_charges_id=<?php echo $_REQUEST['pay_charges_id'];?>" class="nav-link px-3 nav-tab <?php echo esc_html($active_tab) == 'add-payment' ? 'nav-tab-active' : ''; ?>">
				...
				{ 
					...
					{ ?>
						<a href="?apartment-dashboard=user&page=accounts&tab=add-charges" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'add-charges' ? 'active' : ''; ?>">
		  ...
		  <?php		 } 
			...
			<li class="<?php if($active_tab=='expense-list'){?>active<?php }?>">
				<a href="?apartment-dashboard=user&page=accounts&tab=expense-list" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'expense-list' ? 'active' : ''; ?>">
			...
			<li class="<?php if($active_tab=='add-expense'){?>active<?php }?>">
			  <?php 
				if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['expense_id']))
				{ ?>
					<a href="?apartment-dashboard=user&page=accounts&tab=add-expense&action=edit&expense_id=<?php echo $_REQUEST['expense_id'];?>" class="nav-link px-3 nav-tab margin_top_10_res <?php echo esc_html($active_tab) == 'visitor-checkin' ? 'nav-tab-active' : ''; ?>">
				...
				{ 
					if($user_access['add']=='1')
					{ ?>
						<a href="?apartment-dashboard=user&page=accounts&tab=add-expense" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'add-expense' ? 'active' : ''; ?>">
				...
				<table id="invoice_list" class="display" cellspacing="0" width="100%">
					...
					<tbody>
						...
						if($obj_apartment->role=='member')
						{
							...
							if($own_data == '1')
							{
								$invoice_data= $obj_account->MJ_amgt_get_member_all_invoice();	
						...
						elseif($obj_apartment->role=='staff_member')
						{
							...
							if($own_data == '1')
							{  
								$invoice_data= $obj_account->MJ_amgt_get_own_invoice($user_id);	
							}
							else
							{
								$invoice_data= $obj_account->MJ_amgt_get_all_invoice();	
						...
						$obj_amgttax=new MJ_amgt_Tax();
						 ...
						 if(!empty($invoice_data))
						 {
							...
							<tr>	
								...
								<td class="building_name">
									<?php
									if(!empty(get_the_title(get_user_meta($member_id,'building_id',true))))
									{
										echo esc_html(get_the_title(get_user_meta($member_id,'building_id',true)));
									}
									else
									{
										echo "-";
								...
								<td class="unit_name">
									<?php
									if(!empty(get_user_meta($member_id,'unit_name',true)))
									{
										echo esc_html(get_user_meta($member_id,'unit_name',true));
									}
									else
									{
										echo "-";
								  ...
								  <td class="income_type"><?php echo esc_html($charge_cal_by);?></td>
								   <?php 
									...
									?>
								 <td class="building_id"><?php   echo esc_html(MJ_amgt_get_currency_symbol(get_option( 'apartment_currency_code' )).' '.round($total_amount));?></td>
								 <td class="building_id"><?php   echo esc_html(MJ_amgt_get_currency_symbol(get_option( 'apartment_currency_code' )).' '.round(abs($due_amount)));?></td>
								  <td class="building_id"><?php  echo esc_html(MJ_amgt_get_currency_symbol(get_option( 'apartment_currency_code' )).' '.round($paid_amount));?></td>
								  <td class="building_id"><span class="btn btn-success btn-xs"><?php  echo esc_html($payment_status);?></span></td>				
								   <?php 
									...
									{ ?>
										<td class="building_id"><?php  if($retrieved_data->amgt_charge_period=='0'){echo '-'; }else{ echo esc_html(date(MJ_amgt_date_formate(),strtotime($retrieved_data->start_date))); }?>
										</td>
									<?php 
									} 
									...
									{ ?>
										<td class="building_id"><?php  if($retrieved_data->amgt_charge_period=='0'){echo '-'; } else{ echo esc_html(date(MJ_amgt_date_formate(),strtotime($retrieved_data->end_date))); }?>
										</td>
									<?php 
									} 
									?>	
								...
								{									
									...
									{									
									?>					
										<a href="#"  class="show-payment-popup btn btn-success" invoice_id="<?php echo esc_attr($retrieved_data->id); ?>" member_id="<?php echo esc_attr($member_id);?>" view_type="payment" due_amount="<?php echo round($due_amount);?>"><?php esc_html_e('Pay','apartment_mgt');?></a>
									<?php
									}
								}
								?>	
					...
					<tbody>
					 <?php	
						...
						if($obj_apartment->role=='member')
						{
							...
							if($own_data == '1')
							{
								$expensedata=$obj_account->MJ_amgt_get_own_expense($user_id);
							}
							else
							{
			...
			<form name="expense_form" action="" method="post" class="form-horizontal" id="expense_form">
				...
				<div class="form-group">
					<div class="mb-3 row">	
						...
						<div class="col-sm-2 margin_top_10_res"><button class="btn btn-default" id="addremove" model="expense_types"><?php esc_html_e('Add Or Remove','apartment_mgt');?></button></div>
					</div>	
				</div>				
				<div class="form-group">
					<div class="mb-3 row">	
				...
				<tbody>
					 <?php 		
						...
						if($obj_apartment->role=='member')
						{
							...
							if($own_data == '1')
							{
								$chargesdata=$obj_account->MJ_amgt_get_own_charges($user_id);
							}
							else
							{
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				if (is_rtl())
					{
					?>	
						$('#recuring_charges_form').validationEngine({promptPosition : "topLeft",maxErrorsPerField: 1});
					<?php
					}
					else{
						?>
		   ...
		   <form name="recuring_charges_form" action="" method="post" class="form-horizontal" id="recuring_charges_form">
					...
					<div class="mb-3 row">	
						...
						<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
							...
							<label class="radio-inline front_radio">
							  <input type="radio" value="0" class="tog validate[required] radio_border_radius amgt_charge_period_value_one_charge amgt_charge_period_value" name="amgt_charge_period"  <?php  checked( '0', $charge_period);  if($edit){ if($charge_period != '0'){ echo 'disabled="disabled"'; } }?>/><?php esc_html_e('One Time','apartment_mgt');?> 
							</label>
							<label class="radio-inline front_radio">
							  <input type="radio" value="1" class="tog validate[required] radio_border_radius amgt_charge_period_value" name="amgt_charge_period"  <?php  checked( '1', $charge_period);  if($edit){ if($charge_period != '1'){ echo 'disabled="disabled"'; } }?>/><?php esc_html_e('Monthly','apartment_mgt');?>
							</label>
							<label class="radio-inline front_radio">
							  <input type="radio" value="3" class="tog validate[required] radio_border_radius amgt_charge_period_value" name="amgt_charge_period"  <?php  checked( '3', $charge_period);  if($edit){ if($charge_period != '3'){ echo 'disabled="disabled"'; } }?>/><?php esc_html_e('Quarterly','apartment_mgt');?> 
							</label>
							 <label class="radio-inline front_radio margin_left_0_res">
							  <input type="radio" value="12" class="tog  validate[required] radio_border_radius amgt_charge_period_value" name="amgt_charge_period"  <?php  checked( '12', $charge_period);  if($edit){ if($charge_period != '12'){ echo 'disabled="disabled"'; } }?>/><?php esc_html_e('Yearly','apartment_mgt');?> 
							 </label>
						</div>
						</div>
				</div>
				...
				<div class="form-group">
					<div class="mb-3 row">	
						...
						<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
						  ...
						  ?>
							<div class="radio">
								<label class="radio-btn front_radio">
								...
								&nbsp;&nbsp;&nbsp;&nbsp;
								<label class="radio-btn front_radio margin_top_5_res">
				...
				<div class="form-group"><!---FORM-GROUP-->
					<div class="mb-3 row">	
						...
						<div class="col-sm-2 margin_top_10_res"><button class="btn btn-default" id="addremove" model="charges_category"><?php esc_html_e('Add Or Remove','apartment_mgt');?></button></div>
					</div>	
				</div>				
				<?php
		...
		<script>
			...
			function add_entry1()
			{
				...
				if(charge_cal=='fix_charge')
				{
					...
					blank_custom_label+='<button type="button" class="btn btn-danger margin_top_10_res" onclick="deleteParentElement(this)">';
