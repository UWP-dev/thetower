...
<div class="panel-body panel-white"><!--PANEL-WHITE DIV-->
    <ul class="nav nav-tabs panel_tabs" role="tablist"><!-- PANEL_TABS-->
	  	<li class="<?php if($active_tab=='notice_list'){?>active<?php }?>">
			<a href="?apartment-dashboard=user&page=notice-event&tab=notice_list" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'notice_list' ? 'active' : ''; ?>">
       ...
       <li class="<?php if($active_tab=='add_notice'){?>active<?php }?>">
		  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['notice_id']))
			{ ?>
			<a href="?apartment-dashboard=user&page=notice-event&tab=add_notice&action=edit&notice_id=<?php echo $_REQUEST['notice_id'];?>" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'visitor-checkin' ? 'nav-tab-active' : ''; ?>">
			...
			else
			{
				...
				{ ?>
				<a href="?apartment-dashboard=user&page=notice-event&tab=add_notice" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'add_notice' ? 'active' : ''; ?>">
		...
		<li class="<?php if($active_tab=='event_list'){?>active<?php }?>">
				<a href="?apartment-dashboard=user&page=notice-event&tab=event_list" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'event_list' ? 'active' : ''; ?>">
		   ...
		   <li class="<?php if($active_tab=='add_event'){?>active<?php }?>">
			  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['event_id']))
				{ ?>
				<a href="?apartment-dashboard=user&page=notice-event&tab=add_event&action=edit&event_id=<?php echo $_REQUEST['event_id'];?>" class="nav-link px-3 nav-tab margin_top_10_res <?php echo esc_html($active_tab) == 'add_event' ? 'nav-tab-active' : ''; ?>">
				...
				{ 
					if($user_access['add']=='1')
					{?>
						<a href="?apartment-dashboard=user&page=notice-event&tab=add_event" class="nav-link px-3 tab margin_top_10_res <?php echo esc_html($active_tab) == 'add_event' ? 'active' : ''; ?>">
						...
						if(!empty($noticedata))
						{
							...
							{ ?>
								<tr>
									...
									<td class="description"><?php echo esc_html(wp_trim_words( $retrieved_data->description,5));?></td>
									<td class="action">
										<a href="#" class="btn btn-primary view-notice" id="<?php echo esc_attr($retrieved_data->id);?>"> <?php esc_html_e('View','apartment_mgt');?></a>
									<?php
