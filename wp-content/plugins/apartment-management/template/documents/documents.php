...
function fileCheck(obj)
{
	...
	{
		alert("<?php esc_html_e('Sorry, only JPG, JPEG, PNG, GIF, DOCS and PDF files are allowed.','apartment_mgt');?>");
		$(obj).val('');
	}	
}
</script>
...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
{
	...
	$result=$obj_document->MJ_amgt_delete_document($_REQUEST['document_id']);
	if($result)
	{
...
<div class="panel-body panel-white"><!-- PANEL WHITE DIV -->
    <ul class="nav nav-tabs panel_tabs" role="tablist"><!--NAV-TABS-->
	  	<li class="<?php if($active_tab=='documentlist'){?>active<?php }?>">
			<a href="?apartment-dashboard=user&page=documents&tab=documentlist" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'documentlist' ? 'active' : ''; ?>">
        ...
        <li class="<?php if($active_tab=='add_document'){?>active<?php }?>">
		  <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['document_id']))
			{ ?>
			<a href="?apartment-dashboard=user&page=documents&tab=add_document&action=edit&document_id=<?php echo $_REQUEST['document_id'];?>" class="nav-link px-3 nav-tab <?php echo esc_html($active_tab) == 'add_document' ? 'nav-tab-active' : ''; ?>">
			...
			{ 
				...
				{
				?>
					<a href="?apartment-dashboard=user&page=documents&tab=add_document" class="nav-link px-3 tab <?php echo esc_html($active_tab) == 'add_document' ? 'active' : ''; ?>">
							...
							{ ?>
								<tr>
									<td class="title"><!--TITLE--->
										<?php  echo esc_html($retrieved_data->doc_title);?></td>
									<td class="unit"><?php echo esc_html($retrieved_data->unit_name);?></td>
									<td class="member"><?php echo esc_html(MJ_amgt_get_display_name($retrieved_data->member_id));?></td>
									<td class="date"><?php echo esc_html(MJ_amgt_change_dateformat($retrieved_data->created_date));?></td>
