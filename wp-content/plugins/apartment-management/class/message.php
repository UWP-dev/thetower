...
{	
	...
	public function MJ_amgt_add_message($data)
	{
		...
		$subject = MJ_amgt_strip_tags_and_stripslashes($data['subject']);
		$message_body = MJ_amgt_strip_tags_and_stripslashes($data['message_body']);
		$role=$data['receiver'];
		...
		{ 
			...
			{		
				...
				{				
					...
					$first_name = get_user_meta($reciever_data->ID, 'first_name',true);
					$last_name = get_user_meta($reciever_data->ID, 'last_name',true);
	...
	public function MJ_amgt_count_all_send_item()
	{
		global $wpdb;
		$posts = $wpdb->prefix."posts";
		$total =$wpdb->get_var("SELECT Count(*) FROM ".$posts." Where post_type = 'amgt_message'");
		return $total;
	}
	...
	public function MJ_amgt_get_all_send_message($max=10,$offset=0)
	{	
		$args['post_type'] = 'amgt_message';
		$args['posts_per_page'] =$max;
		$args['offset'] = $offset;
