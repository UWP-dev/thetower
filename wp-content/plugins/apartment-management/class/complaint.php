...
{	//ADD COMPLAIN FUNCTION
	public function MJ_amgt_add_complaint($data,$file_name)
	{
		...
		$complaintdata['complaint_description']=MJ_amgt_strip_tags_and_stripslashes($data['description']);
		$complaintdata['complain_title']=MJ_amgt_strip_tags_and_stripslashes($data['complain_title']);
		$complaintdata['created_date']=date('Y-m-d');
		if(!empty($file_name))
		{
			$complaintdata['complain_doc']=$file_name;
		}
		else
		{
		...
		if($data['action']=='edit')
		{
			$complaintdata['complaint_status']=MJ_amgt_strip_tags_and_stripslashes($data['status']);
			$complaintdata['resolution']=MJ_amgt_strip_tags_and_stripslashes($data['resolution']);
		...
		else
		{
			...
			{	
				...
				{
					if(!empty(get_user_meta($data['member_id'], 'phonecode',true))){ $phone_code=get_user_meta($data['member_id'], 'phonecode',true); }else{ $phone_code='+'.MJ_amgt_get_countery_phonecode(get_option( 'amgt_contry' )); }
			...
			else
			{
				...
				if(is_plugin_active('sms-pack/sms-pack.php'))
				{
					...
					{
						if(!empty(get_user_meta($user_id1, 'phonecode',true))){ $phone_code=get_user_meta($user_id1, 'phonecode',true); }else{ $phone_code='+'.MJ_amgt_get_countery_phonecode(get_option( 'amgt_contry' )); }
	...
	public function MJ_amgt_complaint_countby_status_and_user_id($status,$user_id)
	{
		global $wpdb;
		$table_name = $wpdb->prefix. 'amgt_complaints';
		$result = $wpdb->get_var("SELECT count(*) FROM $table_name where complaint_status= '".$status."' AND complaint_member_id=$user_id");
		return $result;
	}
	//GET OWN CRETATED COMPLAIN FUNCTION
