...
{    
				...
				else
				{
					...
					if($result)	
					{
						...
						else
						{
								...
								MJ_amgt_SendEmailNotification($to,$subject,$message_content,$headers);
						}
					}
				}				
		}
	...
	public function MJ_amgt_save_charges_payment_all_member($data)
	{
		...
		$charges_recuring_data['end_date_one_charge']=$data['end_date_one_charge'];
		if(isset($data['one_time_recurring_option']))
		$charges_recuring_data['one_time_recurring_option']=$data['one_time_recurring_option'];	
		...
		{			
			...
			{
				$charges_recuring_data['start_date_one_charge'] = $data['start_date_one_charge'];
			}
	...
	function MJ_amgt_get_chargis_one_time($chargis_option,$recuring_date_new1)
	{
		...
		$result = $wpdb->get_results("SELECT * FROM $table_name where amgt_charge_period=$chargis_option and invoice_end_date <='$recuring_date_new1' AND end_date_one_charge >= '$recuring_date_new1' AND delete_status='0' AND one_time_recurring_option = 1");
		return $result;
	}
