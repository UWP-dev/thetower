...
<?php 
	...
	{		
		...
		{	
			...
			{
				$result=$obj_assets->MJ_amgt_delete_assets($id);
			}
			if($result) 
			{
				wp_redirect ( admin_url() . 'admin.php?page=amgt-assets-inventory&tab=assets_list&message=3');
		...
		else if(isset($_REQUEST['inventory_id']))
		{	
			foreach($_REQUEST['inventory_id'] as $id)
			{
				$result=$obj_assets->MJ_amgt_delete_inventory($id);
			}
			if($result) 
			{
				wp_redirect ( admin_url() . 'admin.php?page=amgt-assets-inventory&tab=inventory_list&message=3');
		...
		if(isset($_REQUEST['message']))
	    {
			...
			{?><div id="message" class="updated below-h2 notice is-dismissible"><p><?php
						esc_html_e("Asset updated successfully.",'apartment_mgt');
						?></p>
						</div>
					<?php 
			...
			elseif($message == 5)
			{?><div id="message" class="updated below-h2 notice is-dismissible"><p><?php
						esc_html_e("Inventory updated successfully.",'apartment_mgt');
						?></p>
						</div>
					<?php 
				...
				<div class="panel panel-white"><!--PANEL-WHITE-->
					<div class="panel-body"><!--PANEL BODY-->
						...
						<h2 class="nav-tab-wrapper"> <!-----NAV-TAB-WRAPPER----->
							<a href="?page=amgt-assets-inventory&tab=assets_list" class="nav-tab <?php echo esc_html($active_tab) == 'assets_list' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Asset List', 'apartment_mgt'); ?></a>
							
							<?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit'  && $_REQUEST['tab'] == 'add_assets')
							{ ?>
							<a href="?page=amgt-assets-inventory&tab=add_assets&action=edit&assets_id=<?php echo $_REQUEST['assets_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add_assets' ? 'nav-tab-active' : ''; ?>">
							<?php esc_html_e('Edit Asset', 'apartment_mgt'); ?></a>  
							<?php 
							}
							else 
							{ ?>
								<a href="?page=amgt-assets-inventory&tab=add_assets" class="nav-tab <?php echo esc_html($active_tab) == 'add_assets' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-plus-alt"></span> '.esc_html__('Add Asset', 'apartment_mgt'); ?></a>
							<?php  } ?>
							
							<a href="?page=amgt-assets-inventory&tab=inventory_list" class="nav-tab <?php echo esc_html($active_tab) == 'inventory_list' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Inventory List', 'apartment_mgt'); ?></a>
							
							<?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && $_REQUEST['tab'] == 'add_inventory')
							{ ?>
							<a href="?page=amgt-assets-inventory&tab=add_inventory&action=edit&inventory_id=<?php echo $_REQUEST['inventory_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add_inventory' ? 'nav-tab-active' : ''; ?>">
							<?php esc_html_e('Edit Inventory', 'apartment_mgt'); ?></a>  
							<?php 
							}
							else 
							{ ?>
								<a href="?page=amgt-assets-inventory&tab=add_inventory" class="nav-tab <?php echo esc_html($active_tab) == 'add_inventory' ? 'nav-tab-active' : ''; ?>">
									...
									<table id="assets_list" class="display" cellspacing="0" width="100%">
										<thead>
											<tr>
												...
												<th><?php esc_html_e('Asset Cost','apartment_mgt'); echo ' '. '('.     MJ_amgt_get_currency_symbol(get_option( 'apartment_currency_code')).')';?></th>
