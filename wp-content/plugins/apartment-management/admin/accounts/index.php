...
<div class="page-inner min_height_1088">
					...
					{							
						...
						if($_POST['amgt_charge_period']=='0')
						{
							if($_POST['one_time_recurring_option'] == 1)
							{
								...
								if($end_date_add <= end_date)
								{
									$invoice_data['end_date']=$end_date;
								}
								else
								{
									$invoice_data['end_date']=$end_date_add;
								}
						...
						{
							if(!empty(get_user_meta($invoice_data['member_id'], 'phonecode',true))){ $phone_code=get_user_meta($invoice_data['member_id'], 'phonecode',true); }else{ $phone_code='+'.MJ_amgt_get_countery_phonecode(get_option( 'amgt_contry' )); }
	...
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
	{
		...
		{
			$result=$obj_account->MJ_amgt_delete_invoice($_REQUEST['memebr_invoice_id']);
			if($result)
			{
		...
		if(isset($_REQUEST['recuring_charges_id']))
		{
			...
			$result=$obj_account->MJ_amgt_delete_recuring_charges($_REQUEST['recuring_charges_id']);
			if($result)
			{
				wp_redirect ( admin_url().'admin.php?page=amgt-accounts&tab=Recurring-list&message=3');
			}
	...
	if(isset($_REQUEST['delete_selected2'])) // multiple check box delete //
	{		
		if(isset($_REQUEST['selected_id']))
		{	
			foreach($_REQUEST['selected_id'] as $id)
			{
				$result=$obj_account->MJ_amgt_delete_invoice($id);
		...
		{	
			foreach($_REQUEST['charge_id'] as $id)
			{
				$result=$obj_account->MJ_amgt_delete_paid_charges($id);
		...
		if(isset($_REQUEST['expense_id']))
		{	
			foreach($_REQUEST['expense_id'] as $id)
			{
				$result=$obj_account->MJ_amgt_delete_expense($id);
	...
	if(isset($_REQUEST['message']))
	{
		...
		{?><div id="message" class="updated below-h2 notice is-dismissible"><p><?php
					esc_html_e("Charge updated successfully.",'apartment_mgt');
					?></p>
					</div>
				<?php 
					...
					<div class="panel-body"><!--PANEL-BODY-->
						...
						<h2 class="nav-tab-wrapper">	 
							<a href="?page=amgt-accounts&tab=invoice-list" 
								class="nav-tab <?php echo esc_html($active_tab) == 'invoice-list' ? 'nav-tab-active' : ''; ?>">
							...
							<a href="?page=amgt-accounts&tab=Recurring-list" 
								class="nav-tab <?php echo esc_html($active_tab) == 'Recurring-list' ? 'nav-tab-active' : ''; ?>">
								<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Charges List', 'apartment_mgt'); ?>
							</a>
							 <?php 
							if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' &&  $_REQUEST['tab'] == 'add-Recurring-Charges')
							{ ?>
								<a href="?page=amgt-accounts&tab=add-Recurring-Charges&action=edit&pay_charges_id=<?php echo $_REQUEST['pay_charges_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add-Recurring-Charges' ? 'nav-tab-active' : ''; ?>">
							...
							{ ?>
								<a href="?page=amgt-accounts&tab=add-Recurring-Charges"	class="nav-tab <?php echo esc_html($active_tab) == 'add-Recurring-Charges' ? 'nav-tab-active' : ''; ?>">
							...
							<a href="?page=amgt-accounts&tab=expense-list" 
								class="nav-tab <?php echo esc_html($active_tab) == 'expense-list' ? 'nav-tab-active' : ''; ?>">
								<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Expense List', 'apartment_mgt'); ?>
							</a>
							<?php  
							if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' &&  $_REQUEST['tab'] == 'add-expense')
							{ ?>
								<a href="?page=amgt-accounts&tab=add-expense&action=edit&expense_id=<?php echo $_REQUEST['expense_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add-expense' ? 'nav-tab-active' : ''; ?>">
							...
							{ ?>
								<a href="?page=amgt-accounts&tab=add-expense" 
									class="nav-tab <?php echo esc_html($active_tab) == 'add-expense' ? 'nav-tab-active' : ''; ?>">
											...
											if(!empty($invoice_data))
											{
												...
												<tr>
													  ...
													  <?php
												        ...
												        ?>
													 <td class="building_id"><?php echo MJ_amgt_get_currency_symbol(get_option( 'apartment_currency_code' )).' '.$total_amount;?></td>
													 
													 <td class="building_id"><?php   echo esc_html(MJ_amgt_get_currency_symbol(get_option( 'apartment_currency_code' )).' '.abs($due_amount));?></td>
													...
													{ ?>
														<td class="building_id"><?php  if($retrieved_data->amgt_charge_period=='5'){echo '-'; }else{ echo '-'; }?>
														</td>
													<?php
													}
													else
													{ ?>
														<td class="building_id"><?php  if($retrieved_data->amgt_charge_period=='5'){echo '-'; }else{ echo esc_html(date(MJ_amgt_date_formate(),strtotime($retrieved_data->start_date))); }?>
														</td>
													<?php 
													} 
													...
													{ ?>
														<td class="building_id"><?php  if($retrieved_data->amgt_charge_period=='5'){echo '-'; }else{ echo esc_html(date(MJ_amgt_date_formate(),strtotime($retrieved_data->end_date))); }?>
														</td>
													<?php 
													} 
													?>	
