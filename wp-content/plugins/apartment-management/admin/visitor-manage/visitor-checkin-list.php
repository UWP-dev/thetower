...
$(document).ready(function() {
	...
	if (is_rtl())
		{
		?>	
			jQuery('#reports').validationEngine({promptPosition : "topLeft",maxErrorsPerField: 1});
		<?php
		}
		else{
			?>
		...
		<form method="post" id="reports">
				<div class="mb-3 row">	
					<div class="form-group w-auto">
						<div class="mb-3 row">	
							<label class="col-sm-1 date_label label_100  control-label form-label margin_top_10 text_align_end" for="facility_start_date">
							<?php esc_html_e('Start Date','apartment_mgt');?> <span class="require-field">*</span></label>
							<div class="col-sm-2 min_width_200_px">
							...
							<label class="col-sm-1 date_label label_100 control-label margin_top_10 text_align_end" for="facility_start_date">
							<?php esc_html_e('Status','apartment_mgt');?> <span class="require-field">*</span></label>
							<div class="col-sm-2">
								<select class="form-select status validate[required]" name="status">
			...
			<form method="post">
				...
				if(isset($_POST['filter_visitor']))
				{
					...
					$visitor_checkindata=$obj_gate->MJ_amgt_get_all_visitor_checkinentries_filter($starting_date,$ending_date,$status);
