<?php
 error_reporting(0);
$active_tab = sanitize_text_field(isset($_GET['tab'])?$_GET['tab']:'manage-gates');
...
{	
	...
	{
		$result=$obj_gate->MJ_amgt_add_gate($_POST);
		if($result)
		{
...
if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')//DELETE VISITOR
{
	...
	{
		$result=$obj_gate->MJ_amgt_delete_visitor_request($_REQUEST['visitor_request_id']);
		if($result)
		{
...
{		
	if(isset($_REQUEST['visitor_request_id']))
	{	
		foreach($_REQUEST['visitor_request_id'] as $id)
		{
			$result=$obj_gate->MJ_amgt_delete_visitor_checkin_entry($id);
				...
				<div class="panel panel-white"><!--PANEL-WHITE-->
					<div class="panel-body"><!--PANEL BODY-->
						<h2 class="nav-tab-wrapper"><!--NAV TAB WRAPPER----->
							<a href="?page=amgt-visiter-manage&tab=manage-gates" class="nav-tab <?php echo esc_html($active_tab) == 'manage-gates' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Manage Gates', 'apartment_mgt'); ?></a>
							
							<a href="?page=amgt-visiter-manage&tab=visitor-checkinlist" class="nav-tab <?php echo esc_html($active_tab) == 'visitor-checkinlist' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Visitor Request List', 'apartment_mgt'); ?></a>  
							
							 <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && $active_tab == 'visitor-checkin')
							{ ?>
									<a href="?page=amgt-visiter-manage&tab=visitor-checkin&action=edit&visitor_checkin_id=<?php echo $_REQUEST['visitor_checkin_id']?>" class="nav-tab <?php echo esc_html($active_tab) == 'visitor-checkin' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-plus-alt"></span> '.esc_html__('Edit Visitor Request', 'apartment_mgt'); ?></a>
							<?php } 
							else
							{ ?>
							<a href="?page=amgt-visiter-manage&tab=visitor-checkin" class="nav-tab <?php echo esc_html($active_tab) == 'visitor-checkin' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-plus-alt"></span> '.esc_html__('Visitor Request', 'apartment_mgt'); ?></a>
							<?php } ?>
							
							<a href="?page=amgt-visiter-manage&tab=staff-checkinlist" class="nav-tab <?php echo esc_html($active_tab) == 'staff-checkinlist' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Staff Checkin List', 'apartment_mgt'); ?></a>
							
							 <?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && $active_tab == 'staff-checkin')
							{ ?>
									<a href="?page=amgt-visiter-manage&tab=staff-checkin&action=edit&staff_checkin_id=<?php  $_REQUEST['staff_checkin_id']?>" class="nav-tab <?php echo esc_html($active_tab) == 'staff-checkin' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-plus-alt"></span> '.esc_html__('Edit Staff Checkin', 'apartment_mgt'); ?></a>
							<?php } 
							else
							{?>
								<a href="?page=amgt-visiter-manage&tab=staff-checkin" class="nav-tab <?php echo esc_html($active_tab) == 'staff-checkin' ? 'nav-tab-active' : ''; ?>">
								...
								<form name="gate_form" action="" method="post" class="form-horizontal" id="gate_form">
										<div class="form-group">
											<div class="mb-3 row">	
												...
												<div class="col-md-3 col-sm-3 col-xs-4 row">
												...
												<div id="<?php echo $i;?>">
													<div class="form-group">
														<div class="mb-3 row">	
															...
															<input type="hidden" name="gate_id_<?php echo $i;?>" value="<?php echo esc_attr($gate->id);?>">
															</div>
															<div class="col-md-3 col-sm-3 col-xs-4 row">
																<div class="col-md-6 col-sm-6 col-xs-6">
																<input id="forentry" class="margin_top_12px form-control forentry" type="checkbox" <?php checked('yes',$gate->for_entry);?> value="yes" name="for_entry_<?php echo $i;?>" >
																</div>
																<div class="col-md-6 col-sm-6 col-xs-6">
																<input id="forexit" class="margin_top_12px form-control forexit" type="checkbox" <?php checked('yes',$gate->for_exit);?> value="yes" name="for_exit_<?php echo $i;?>">
																</div>
															</div>
															
															
															<div class="col-sm-4 col-md-4 col-xs-3">
															<button id="del_curr" type="button" class="btn btn-danger" test_id="<?php echo $i;?>" gate_id="<?php echo $gate->id;?>" >
												...
												<div class="form-group">
													<div class="mb-3 row">	
														...
														<div class="col-sm-3 row">
															<div class="col-md-6">
															<input id="forentry" class="margin_top_12px form-control forentry" type="checkbox" value="yes" name="for_entry_<?php echo $i;?>" >
															</div>
															<div class="col-md-6">
															<input id="forexit" class="margin_top_12px form-control forexit" type="checkbox" value="yes" name="for_exit_<?php echo $i;?>">
															</div>
														</div>
		...
		data +='<div id="'+counter+'">';
				data +='<div class="form-group">';
					data +='<div class="mb-3 row">';
						...
						data +='<div class="col-sm-3 row">';
							data +='<div class="col-md-6">';
							data +='<input id="forentry" class="margin_top_12px form-control forentry" type="checkbox" value="yes" name="for_entry_'+counter+'" >';
							data +='</div>';
							data +='<div class="col-md-6">';
							data +='<input id="forexit" class="margin_top_12px form-control forexit" type="checkbox" value="yes" name="for_exit_'+counter+'">';
							data +='</div>';
						data +='</div>';
						data +='<div class="col-sm-1">';
						data +='<button id="del_curr" type="button" class="btn btn-danger" test_id='+counter+'><i class="entypo-trash"><?php echo __("Delete","apartment_mgt")?>';
						data += '</i></button>';
						data += '</div>';
						data += '</div>';	
						data += '</div>';	
