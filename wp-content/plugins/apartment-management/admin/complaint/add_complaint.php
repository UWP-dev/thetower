	...
	if ($.inArray($(obj).val().split('.').pop().toLowerCase(), fileExtension) == -1)
	{
     ...
     <?php 
		...
		<div class="panel-body"><!--PANEL BODY-->
            ...
            <form name="complaint_form" action="" method="post" class="form-horizontal" id="complaint_form" enctype="multipart/form-data" >
					...
					<div class="mb-3 row">	
						...
						<?php $complaintval = "complaint"; if($edit){ $complaintval=$result->complaint_nature; }elseif(isset($_POST['complaint_nature'])) {$complaintval=$_POST['complaint_nature'];}?>
							<label class="radio-inline">
							 <input type="radio" value="complaint" class="tog complaint_nature123" name="complaint_nature"  <?php  checked( 'complaint', $complaintval);  ?>/><?php esc_html_e('Complaint','apartment_mgt');?>
							</label>
							<label class="radio-inline">
							  <input type="radio" value="suggestion" class="tog complaint_nature123" name="complaint_nature"  <?php  checked( 'suggestion', $complaintval);  ?>/><?php esc_html_e('Suggestion','apartment_mgt');?> 
							</label>
							 <label class="radio-inline">
							  <input type="radio" value="request" class="tog complaint_nature123" name="complaint_nature"  <?php  checked( 'request', $complaintval);  ?>/><?php esc_html_e('Request','apartment_mgt');?> 
							</label>
							
							<label class="radio-inline">
							  <input type="radio" value="Maintenance Request" class="tog complaint_nature123" name="complaint_nature"  <?php  checked( 'Maintenance Request', $complaintval);  ?>/><?php esc_html_e('Maintenance Request','apartment_mgt');?> 
							</label>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					<div class="mb-3 row">	
						...
						<div class="col-sm-8">
							 <input id="complain_title" maxlength="50" class="form-control text-input validate[required,custom[onlyLetter_specialcharacter]]" type="text"  value="<?php if($edit){ echo esc_attr($result->complain_title);}elseif(isset($_POST['complain_title'])) echo esc_attr($_POST['complain_title']);?>" name="complain_title">
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					<div class="mb-3 row">	
						...
						<?php $complaintval = "individual"; if($edit){ $complaintval=$result->complaint_type; }elseif(isset($_POST['type'])) {$complaintval=$_POST['type'];}?>
							<label class="radio-inline">
							 <input type="radio" id="type_individual" value="individual" class="tog complain_type" name="type"  <?php  checked( 'individual', $complaintval);  ?>/><?php esc_html_e('Individual','apartment_mgt');?>
							</label>
							<label class="radio-inline">
							  <input type="radio" id="type_society" value="society" class="tog complain_type" name="type"  <?php  checked( 'society', $complaintval);  ?>/><?php esc_html_e('Compound','apartment_mgt');?> 
							</label>
						</div>
					</div>
				</div>
				...
				<div class="form-group single_member" style="<?php echo $members_bloack; ?>">
					<div class="mb-3 row">	
						<label class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label form-label" for="day"><?php esc_html_e('Member','apartment_mgt');?><span class="require-field">*</span></label>	
						<div class="col-sm-8">
							...
							<option value=""><?php esc_html_e('Select Member','apartment_mgt');?></option>
								...
								if(!empty($membersdata))
								{
									...
									{
										 echo '<option value="'.$retrive_data->ID.'" '.selected($memberid,$retrive_data->ID).'>'.MJ_amgt_get_display_name($retrive_data->ID) .'  -  ' .$retrive_data->unit_name .' </option>';
									}
								} ?>
						</select>
						</div>
			...
			<div class="category_div" style="<?php echo $category_div;?>" >
					<div class="form-group"><!--CATEGORY-->
							<div class="mb-3 row">	
								...
								<div class="col-sm-2"><button class="btn btn-default" id="addremove" model="complaint_category"><?php esc_html_e('Add Or Remove','apartment_mgt');?></button></div>
							</div>	
						</div>
			...
			<div class="time_date_div" style="<?php echo $time_date_div_css;?>">
				<div class="form-group">
					<div class="mb-3 row"> 
				...
				<div class="form-group">
					<div class="mb-3 row">	
						...
						<div class="col-sm-8">
							<input type="text" value="<?php if($edit){ echo esc_attr($result->time);}elseif(isset($_POST['time'])) echo esc_attr($_POST['time']);?>" class="form-control timepicker start" autocomplete="off" name="time" />
						</div>
					</div>
				</div>
			</div>
				...
				<div class="form-group">
					<div class="mb-3 row">	
						...
						<div class="col-sm-8">
							 <textarea name="resolution" maxlength="150" id="description" class="form-control text-input"><?php if($edit) echo esc_textarea($result->resolution);?></textarea>
						</div>
					</div>
				</div>
				<?php } ?>
