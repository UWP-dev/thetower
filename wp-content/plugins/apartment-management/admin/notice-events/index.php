	...
	if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')//DELETE NOTICE
		{
			...
			{
				$result=$obj_notice->MJ_amgt_delete_notice($_REQUEST['notice_id']);
				if($result)
				{
			...
			{
				$result=$obj_notice->MJ_amgt_delete_event($_REQUEST['event_id']);
				if($result)
				{
					wp_redirect ( admin_url().'admin.php?page=amgt-notice-event&tab=event_list&message=10');
				}
			}
	    }
		...
		{		
			...
			{	
				...
				{
					$result=$obj_notice->MJ_amgt_delete_event($id);
				
				}
				if($result) 
				{
					wp_redirect ( admin_url() . 'admin.php?page=amgt-notice-event&tab=event_list&message=11');
				}
			}
			if(isset($_REQUEST['notice_id']))
			{	
				foreach($_REQUEST['notice_id'] as $id)
				{
					$result=$obj_notice->MJ_amgt_delete_notice($id);
		...
		if(isset($_REQUEST['action'])&& $_REQUEST['action']=='approve_notice')
		{
			if(isset($_REQUEST['notice_id']))
			{
				$result=$obj_notice->MJ_amgt_approve_notice($_REQUEST['notice_id']);
				if($result)
				{
			...
			if(isset($_REQUEST['event_id']))
			{
				$result=$obj_notice->MJ_amgt_approve_event($_REQUEST['event_id']);
				if($result)
				{
		...
		if(isset($_REQUEST['message']))//MESSAGE
	     {
			...
			{?><div id="message" class="updated below-h2 notice is-dismissible"><p><?php
						esc_html_e("Notice updated successfully.",'apartment_mgt');
						?></p>
						</div>
					<?php 
			...
			<div id="message" class="updated below-h2 notice is-dismissible"><p>
			<?php 
				esc_html_e('Event deleted successfully','apartment_mgt');
			?></div></p><?php
				...
				<div class="panel panel-white"><!---PANEL WHITE------>	
					<div class="panel-body"><!---PANEL BODY------>
						...
						<h2 class="nav-tab-wrapper">
							<a href="?page=amgt-notice-event&tab=notice_list" class="nav-tab <?php echo esc_html($active_tab) == 'notice_list' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Notice List', 'apartment_mgt'); ?></a>
							
							<?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['notice_id']))
							{ ?>
							<a href="?page=amgt-notice-event&tab=add_notice&action=edit&notice_id=<?php echo $_REQUEST['notice_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add_notice' ? 'nav-tab-active' : ''; ?>">
							<?php esc_html_e('Edit Notice', 'apartment_mgt'); ?></a>  
							<?php 
							}
							else 
							{ ?>
								<a href="?page=amgt-notice-event&tab=add_notice" class="nav-tab <?php echo esc_html($active_tab) == 'add_notice' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-plus-alt"></span> '.esc_html__('Add Notice', 'apartment_mgt'); ?></a>
							<?php  } ?>
							
							<a href="?page=amgt-notice-event&tab=event_list" class="nav-tab <?php echo esc_html($active_tab) == 'event_list' ? 'nav-tab-active' : ''; ?>">
							<?php echo '<span class="dashicons dashicons-menu"></span> '.esc_html__('Event List', 'apartment_mgt'); ?></a>
							
							<?php  if(isset($_REQUEST['action']) && sanitize_text_field($_REQUEST['action']) == 'edit' && isset($_REQUEST['event_id']))
							{ ?>
							<a href="?page=amgt-notice-event&tab=add_event&action=edit&event_id=<?php echo $_REQUEST['event_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add_event' ? 'nav-tab-active' : ''; ?>">
							<?php esc_html_e('Edit Event', 'apartment_mgt'); ?></a>  
							<?php 
							}
							else 
							{ ?>
								<a href="?page=amgt-notice-event&tab=add_event" class="nav-tab <?php echo esc_html($active_tab) == 'add_event' ? 'nav-tab-active' : ''; ?>">
							 ...
							 <thead>
								...
								<tbody>
									 ...
									 if(!empty($noticedata))
									 {
										...
										<tr>
											<td class="title"><input type="checkbox" name="notice_id[]" class="sub_chk" value="<?php echo esc_attr($retrieved_data->id); ?>"></td>
											...
											<td class="valid date"><?php echo esc_html(date(MJ_amgt_date_formate(),strtotime($retrieved_data->valid_date)));?></td>
											...
											<td class="status"><?php if($retrieved_data->status=='Open'){ esc_html_e('Approved','apartment_mgt');}elseif($retrieved_data->status=='Not Approved'){  esc_html_e('Not Approved','apartment_mgt');}?></td>
