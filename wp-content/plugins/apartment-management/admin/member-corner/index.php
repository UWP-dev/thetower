<?php
$active_tab = sanitize_text_field(isset($_GET['tab'])?$_GET['tab']:'rule_list');
...
//---------------- SAVE COMPLAINT --------------------//
	if(isset($_POST['save_complaint']))		
	{
		...
		{
			$result=$obj_complaint->MJ_amgt_add_complaint($_POST);
			if($result)
			{
				...
				<div class="panel panel-white"><!--PANEL WHITE-->
					<div class="panel-body"><!--PANEL BODY-->
						...
						<h2 class="nav-tab-wrapper">
							<a href="?page=amgt-member-corner&tab=rule_list" class="nav-tab <?php echo esc_html($active_tab) == 'rule_list' ? 'nav-tab-active' : ''; ?>">
							...
							{ ?>
							<a href="?page=amgt-member-corner&tab=add_rule&action=edit&complaint_id=<?php echo $_REQUEST['complaint_id'];?>" class="nav-tab <?php echo esc_html($active_tab) == 'add_rule' ? 'nav-tab-active' : ''; ?>">
							<?php esc_html_e('Edit Rule', 'apartment_mgt'); ?></a>  
							<?php 
							}
							else 
							{ ?>
								<a href="?page=amgt-member-corner&tab=add_rule" class="nav-tab <?php echo esc_html($active_tab) == 'add_rule' ? 'nav-tab-active' : ''; ?>">
