					...
					<div class="form-group">
						<div class="mb-3 row">	
							...
							<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
								<input id="sloat_name" maxlength="20" class="form-control validate[required,custom[address_description_validation]] text-input" type="text"  value="<?php if($edit){ echo esc_attr($result->sloat_name);}elseif(isset($_POST['sloat_name'])) echo esc_attr($_POST['sloat_name']);?>" name="sloat_name">
							</div>
						</div>
					</div>
						...
						<div class="mb-3 row">	
							...
							<?php $sloattype = "member"; if($edit){ $sloattype=$result->sloat_type; }elseif(isset($_POST['sloat_type'])) {$sloattype=$_POST['sloat_type'];}?>
								<label class="radio-inline">
								 <input type="radio" value="member" class="tog validate[required]" name="sloat_type"  <?php  checked( 'member', $sloattype);  ?>/><?php esc_html_e('Member','apartment_mgt');?>
								</label>
								<label class="radio-inline">
								  <input type="radio" value="Staff" class="tog validate[required]" name="sloat_type"  <?php  checked( 'Staff', $sloattype);  ?>/><?php esc_html_e('Staff','apartment_mgt');?> 
								</label>
							</div>
						</div>
					</div>
